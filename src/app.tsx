...
function reducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    case 'START_ALL':
      return { ...state, mode: 'running', queue: allSteps, currentIndex: 0 };
    case 'START_CATEGORY': {
      const cat = categories.find(c => c.id === action.categoryId);
      return { ...state, mode: 'running', queue: cat?.steps ?? [], currentIndex: 0 };
    }
    case 'RECORD_OUTCOME': {
...
export default function App({ dryRun, yolo, filterCategory }: Props) {
  ...
  const handleSelectCategory = useCallback((id: StepCategory | 'all') => {
    if (id === 'all') dispatch({ type: 'START_ALL' });
    else dispatch({ type: 'START_CATEGORY', categoryId: id });
  }, []);
